...
//		return;
//
//	fprintf(stderr, "received valid enternotify\n");
//
...
//	}
//
//	fprintf(stderr, "past c is current\n");
//
...
//	}
//
//	fprintf(stderr, "past justmanageunder\n");
//
...
//	}
//
//	fprintf(stderr, "past justswitch\n");
//
//	if ( (m = c->mon) && m != curmon )
//		changemon(m, NoFocus);
//
...
//	updatefocus();
	...
	if (m != curmon){
		...
		curmon = m;
	} else if (!c || c == curmon->current)
		return;

	if (!c || !ISVISIBLE(c))
		c = findvisclient(curmon->head, YesFloat);
	if (curmon->current && curmon->current != c)
		if (curmon->current){
			grabbuttons(c, 0);
			XSetInputFocus(dis, root, RevertToPointerRoot, CurrentTime);
		}
...
void movefocus(const Arg arg){
	...
	if (c && c != curmon->current){
		fprintf(stderr, "justswitch is %d, setting it to 1\n", justswitch);
		justswitch = 1;
		changecurrent(c, curmon, curmon->curdesk);
		updatefocus();
	}
...
void manipulate(const Arg arg){
	...
	if (ISOUTSIDE(c->x, c->y, curmon->x, curmon->y - curmon->bar->h,
				curmon->w, curmon->h + curmon->bar->h))
	{
		for EACHMON(mhead){
			if (im != curmon && !ISOUTSIDE(c->x, c->y, im->x,
						im->y - im->bar->h, im->w, im->h + im->bar->h))
			{
				sendmon(c, im);
				changemon(im, YesFocus);
				return;
