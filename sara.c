...
// TODO: ptr in float, move clients through stack below it, focus drawn to float
/* some dwm copypasta */
void enternotify(XEvent* e){
	...
	/* if focus is pulled, and you enter that client, allow new enternotify events */
	if (c == curmon->current){
		if (justmanage)
	...
	/* if enternotify is generated by movefocus, ignore it */
//	if (justfocus){
//		justfocus = 0;
//		return;
//	}
//
	/* if enternotify is generated by moveclient or view, ignore it */
	if (justswitch){
		justswitch = 0;
		return;
	}
...
void detach(client* c){
	...
	if (c->desks & curmon->curdesk) refocus(c->next, c);
	else changecurrent(findprevclient(c, OnlyVis), curmon->curdesk);
...
void refocus(client* n, client* p){
	client* vis;
	vis = (vis = findvisclient(n, YesFloat)) ? vis : findprevclient(p, OnlyVis);
